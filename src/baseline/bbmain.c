...
static coord_t		distance (coord_t x1, coord_t y1, coord_t x2, coord_t y2);
...
/*################################################################################*/
	...
	for (i = 0; i < cinfo.num_edges; i++) {
		...
		terms = fsp -> terminals;
/*
		(void) fprintf (fp, " %% fs%d:", fsp -> tree_num);
		n = terms -> n;
		p1 = &(terms -> a [0]);
		for (j = 0; j < n; p1++, j++) {
			(void) fprintf (fp, " %lu", (int32u) (p1 -> pnum));
		}
		(void) fprintf (fp,"\n");
*/
		/*draw_fst (fsp, cip);*/
		...
		ep = fsp -> edges;
		for (k = 0; k < fsp -> nedges; k++, ep++) {
			j = ep -> p1;
			if (j < nt) {
				p1 = &(terms -> a [j]);
				/*(void) fprintf (fp,"\t%d T", p1 -> pnum);*/
				coord_to_string (buf1, p1 -> x, &(cinfo.scale));
				coord_to_string (buf2, p1 -> y, &(cinfo.scale));
				(void) fprintf (fp,"%s\t%s\t", buf1, buf2);
				p2=p1;
			}
			else {
				p1 = &(steins -> a [j - nt]);
			...
			/*if(distance(p1->x,p1->y,p2->x,p2->y)>longest)longest=distance(p1->x,p1->y,p2->x,p2->y);*/
		}
	}
	/*coord_to_string (buf3, longest, &(cinfo.scale));*/
...
/*################################################################################*/
		...
		sprintf (title,
			 "%s:  %lu points,  length = %s,  bottleneck = %s,  %s seconds",
			 descr, cinfo.num_verts, buf2, buf3, buf1);
...
{
	double myscale=0.000000000001;
	printf ("%% distance: %lf %lf %lf %lf->%lf\n",x1,y1,x2,y2,myscale*sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)));
